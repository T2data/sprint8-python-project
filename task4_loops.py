...
japanSale = 0
for game in video_game_sales:
    northAmericSale += game[NA_SALES]
    japanSale += game[JP_SALES]
print(f"northAmericSale: {northAmericSale} and japanSale: {japanSale}")
if(japanSale > northAmericSale):
    print("Japan has the higher sales")
else:
    print("North America  has the higher sales")

nintendo_games = []
for game in video_game_sales:
    if game[PUBLISHER] == 'Nintendo':
        nintendo_games.append(game[NAME])
length,nintendo_games = len(nintendo_games),nintendo_games
print(length)
