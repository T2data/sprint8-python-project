...
def calculate_total_sales(game):
    totalSale =  game[NA_SALES] + game[EU_SALES]+ game[JP_SALES]
    return totalSale
...
def filter_by_genre(data, genre='Platform'):
    ...
    return gameList
filter_by_genre(video_game_sales)
print(gameList)
filter_by_genre(video_game_sales,'Role-Playing')
print(gameList)

def get_summary(game):
    for g in game:
        result = f"{g[NAME]} ({g[YEAR]}) - {g[GENRE]} - ${g[GLOBAL_SALES]}M"
        print(result)
print('summary')
get_summary(video_game_sales)

#print(get_summary(video_game_sales))